...
class Dynamics
{
    ...
    const int collision_nodes_count;

    const float dt;       /**< Time between each frame of the simulation, i.e. 24 fps => dt = 0.4 */
    const float alpha;    /**< Wave damping factor. Realistic damping is 0.2 <= alpha <= 0.4, suggested is 0.3 */
    const float gravity;  /**< 9.8 m/s^2 * dt * dt */
    const float sigma;    /**< Used for strength of Gaussian smoothing of obstructions. sigma > 0.0, suggested is 1.0 */
    const float wake_exp; /**< Power of waves generated by an object's wake. wave_exp >= 1.0, suggested is 2.0 */

    const int vertices_x;
    const int vertices_y;
    const int vertices_total;
